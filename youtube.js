      ...
      try {
        if (this.shouldShowFeedItem_(this.feedItems_[i], this.options_)) {
          var child = this.buildFeedItemElement_(feedEntryTemplate,
                                                 this.feedItems_[i]);
          // console.info(child);
          // console.info(this.feedItems_[i]);
          if (child) {
...
YouTube.prototype.onFeedReceived_ = function(text, xhr) {
  ...
  if (data && data['feed'] && data['feed']['entry']) {
    ...
    for (var i = 0; i < feedItems.length; ++i) {
      ...
      var itemId = feedItem['yt$videoid'] && feedItem['yt$videoid']['$t'] + feedItem['updated'] && feedItem['updated']['$t'];
    ...
    // console.info(this.feedItems_);
    // console.info(this.feedMap_);

    for (var index in this.feedItems_) {
      // console.info(this.feedItems_[index]['title']['$t']);
    }
...
YouTube.prototype.getEventType_ = function(feedItem) {
  ...
  for (var i = 0; i < categories.length; ++i) {
    if (categories[i]['scheme'] == YouTube.EVENT_KIND_SCHEME) {
